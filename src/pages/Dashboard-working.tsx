...
const fetchUserData = async (userId: string) => {
  // In production, this would fetch from your backend (Supabase, etc.)
  const response = await fetch(`/api/user/${userId}`);
  return response.json();
};

const fetchProjects = async (userId: string, role: string) => {
...
export default function DashboardPage() {
  ...
  const userName = user?.name || 'Steve';
  const userEmail = user?.email || 'steve@example.com';
  const userAvatar = user?.name ? user.name.charAt(0).toUpperCase() + user.name.charAt(1) : 'ST';
  ...
  useEffect(() => {
    const loadUserData = async () => {
      ...
      try {
        ...
        const [projectsData, tasksData, skillsData] = await Promise.all([
          fetchProjects(user.id, activeRole),
          fetchTasks(user.id, activeRole),
          fetchSkills(user.id)
        ]);
        ...
        if (activeRole === 'freelancer') {
          const gigsResponse = await fetch('/api/gigs?userId=' + user.id);
          setGigs(await gigsResponse.json() || []);
        } else if (activeRole === 'recruiter') {
          const candidatesResponse = await fetch('/api/candidates?userId=' + user.id);
        ...
        <div style={{ padding: '1rem', borderTop: '1px solid #e5e7eb', backgroundColor: '#f8fafc' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <div>
              ...
              <p style={{ color: '#6b7280', fontSize: '0.75rem' }}>{userEmail}</p>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', marginLeft: sidebarOpen ? '280px' : '0px', transition: 'margin-left 0.3s ease' }}>
        ...
        <header style={{ 
          ...
          <div style={{ 
            ...
            {/* File Upload - Hidden on mobile, icon only on tablet */}
            <input
              ref={fileInputRef}
              type="file"
            ...
            <button
              ...
              title={window.innerWidth <= 768 ? 'Upload Files' : ''}
            >
              <span style={{ fontSize: '1.25rem' }}>ð</span>
              {window.innerWidth > 768 && <span>Upload</span>}
            </button>
            ...
            <button 
              ...
              title={window.innerWidth <= 768 ? 'Notifications' : ''}
            >
              <span style={{ fontSize: '1.25rem' }}>ð</span>
              {window.innerWidth > 768 && <span>Notifications</span>}
              {notificationsOpen && (
                <span style={{ 
            ...
            <button 
              ...
              title={window.innerWidth <= 768 ? 'Settings' : ''}
            >
